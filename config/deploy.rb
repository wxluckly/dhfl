...
set :application, 'dhfl'

set :scm, :git
set :repo_url, 'https://github.com/wxluckly/dhfl.git'

set :deploy_to, '/www/dhfl'
  ...
  after :restart, :clear_cache do
    on roles(:web), in: :groups, limit: 3, wait: 10 do
      within current_path do
        execute "/bin/bash --login -c 'god restart dhfl'"
      end
    end
  end
...
namespace :puma do
  ...
  task :start do
    on roles(:web), in: :groups, limit: 3, wait: 10 do
      execute "/bin/bash --login -c 'god start dhfl'"
    end
  end

  task :stop do
    on roles(:web), in: :groups, limit: 3, wait: 10 do
      execute "/bin/bash --login -c 'god stop dhfl'"
    end
  end
